...
import {
  writeJsonToHandle, writeCsvToHandle, parseCsvText,
  loadProjectFiles, reopenLastProject,
...
function applyProjectData(data: Awaited<ReturnType<typeof loadProjectFiles>>, get: () => AppState) {
  if (!data) return;
  console.log('[store] applyProjectData:', data.projectName, '— recipes:', Object.keys(data.config.data).length, '— warnings:', data.warnings);
  const recipeCount = Object.keys(data.config.data).length;
  const criticalWarnings = data.warnings.filter((w) => w !== 'csv_missing');
  const hasPluginIssue   = data.warnings.some((w) => w === 'csv_missing' || w === 'plugin_missing');
  ...
  useStore.setState({
    ...
    csvDirty:       false, csvSnapshot: null,
    dirty:          false, dirtyKeys: new Set(),
    snapshots:      {}, snapshotKeys: {},
    currentKey:     null, loading: false,
  });

  if (warningsToShow.length) {
    useStore.setState({ missingFilesWarnings: warningsToShow, missingFilesOpen: true });
  } else {
    get().addToast(`"${data.projectName}" — ${data.items.length} items, ${recipeCount} recipes`, 'ok');
  }
}
...
export const useStore = create<AppState>((set, get) => ({
  ...
  openProject: async () => {
    ...
    try {
      ...
      console.log('[store] loadProjectFiles returned:', data ? data.projectName : 'null');
  ...
  discardRecipe: (key) => {
    set((s) => {
      ...
      return { config: { ...s.config, data: reordered }, currentKey: originalKey, dirtyKeys: nextDirtyKeys, snapshots: nextSnaps, snapshotKeys: nextSnapshotKeys, dirty: nextDirtyKeys.size > 0 };
    });
  ...
  createRecipe: async (item, cat, qty) => {
    const s = get();
    if (s.config.data[item]) { get().addToast(`A recipe for "${item}" already exists`, 'err'); return; }
    const newConfig: CraftingConfig = { ...s.config, data: { ...s.config.data, [item]: { ingredients: {}, result: item, quantity: qty, category: cat || 'all', unlock_condition: { type: 'manual', value: true } } } };
  ...
  deleteRecipe: async (key) => {
    const s = get();
    const newData = { ...s.config.data }; delete newData[key];
  ...
  renameRecipe: (oldKey, newKey) => {
    ...
    if (!newDirtyKeys.has(newKey) && s.config.data[oldKey]) { newSnaps[newKey] = JSON.parse(JSON.stringify(s.config.data[oldKey])); newDirtyKeys.add(newKey); }
  ...
  updateRecipeField: (key, field, value) => {
    ...
    set((s) => ({ config: { ...s.config, data: { ...s.config.data, [key]: { ...s.config.data[key], [field]: value } } } }));
    get().checkRevert(key);
  },
  ...
  updateIngredient: (recipeKey, oldItem, newItem, qty) => {
    const s = get();
    const ingr = { ...(s.config.data[recipeKey].ingredients || {}) };
    if (newItem !== null && newItem !== oldItem) { const v = ingr[oldItem]; delete ingr[oldItem]; ingr[newItem] = v; }
  ...
  setRootCondition: (recipeKey, type) => {
    const defaults: Record<string, Condition> = { manual: { type: 'manual', value: true }, switch: { type: 'switch', id: 0 }, variable: { type: 'variable', id: 0, value: 0 }, recipe: { type: 'recipe', key: '' }, operator: { operator: 'and', conditions: [] } };
  ...
  addChildCondition: (recipeKey, path, type) => {
    get().snapshotIfClean(recipeKey);
    const defaults: Record<string, Condition> = { manual: { type: 'manual', value: true }, switch: { type: 'switch', id: 0 }, variable: { type: 'variable', id: 0, value: 0 }, recipe: { type: 'recipe', key: '' } };
    set((s) => {
      const recipe = s.config.data[recipeKey];
      const cond: Condition = JSON.parse(JSON.stringify(recipe.unlock_condition));
  ...
  addCategory: async (key, id, name) => {
    const s = get();
    if (s.config.categories.find((c) => Object.keys(c)[0] === key)) { get().addToast('Category already exists', 'err'); return; }
    const newCategories: Category[] = [...s.config.categories, { [key]: id }];
    const newConfig = { ...s.config, categories: newCategories };
    let newCsvLines = [...s.csvLines]; let newCsvTexts = { ...s.csvTexts };
    if (s.csvHandle !== null || name) { // csvHandle est null mais on continue
      while (newCsvLines.length <= id) newCsvLines.push('');
      newCsvLines[id] = name;
      const parsed = parseCsvText(newCsvLines.join('\n'));
      newCsvTexts = parsed.texts; newCsvLines = parsed.lines;
      try { await writeCsvToHandle(null, newCsvLines); } catch (e: any) { get().addToast('CSV write error: ' + e.message, 'warn'); }
    }
  ...
  updateCategoryId: (idx, val) => {
    set((s) => { const cats = [...s.config.categories]; const key = Object.keys(cats[idx])[0]; cats[idx] = { [key]: val }; return { config: { ...s.config, categories: cats } }; });
    get().markDirty(null);
  },
  ...
  deleteCategory: async (idx) => {
    ...
    const newData = { ...s.config.data }; let migrated = 0;
    for (const [rk, recipe] of Object.entries(newData)) {
      if (recipe.category === key) { const { category: _, ...rest } = recipe; newData[rk] = rest as typeof recipe; migrated++; }
    }
